// Marked is loaded from CDN before this script
declare const marked: { parse: (md: string) => string };

interface Post {
  slug: string;
  title: string;
  date: string;       // ISO 8601, e.g. "2026-02-19"
  dateLabel: string;  // Display label, e.g. "Feb 19, 2026"
  tag: string;
  excerpt: string;
  readTime: string;
  counterKey: string; // counterapi.dev key
}
...
function renderPostList(containerId: string): void {
...
function renderPost(): void {
  ...
  fetch(`posts/${slug}.md`)
    ...
    .then((md) => {
      ...
      if (contentEl) contentEl.innerHTML = marked.parse(md);
    })
